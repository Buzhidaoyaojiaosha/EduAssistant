...
class KnowledgePointService:
    ...
    @staticmethod
    def create_knowledge_point(name: str, course_id: int, description: str = None,
                               parent_id: int = None) -> KnowledgePoint:
        """创建一个新的知识点。
        ...
        for _, row in df.iterrows():
            ...
            try:
                if name_lvl1:
                    ...
                    # 创建一级知识点（无父）
                    kp1 = KnowledgePointService.create_knowledge_point(name=name_lvl1, course_id=course_id,
                    ...
                    # 创建二级知识点，父为最近一级
                    kp2 = KnowledgePointService.create_knowledge_point(name=name_lvl2, course_id=course_id,
                                                                       description=None,
                    ...
                    # 创建三级知识点，父为最近二级
                    kp3 = KnowledgePointService.create_knowledge_point(name=name_lvl3, course_id=course_id,
                                                                       description=description,
    ...
    def excel_to_knowledge_point_graph(file, course_id, id_cache):
        ...
        #         MATCH (c:Course {id: $course_id})<-[:BELONGS_TO]-(k:Knowledge)
        #         DETACH DELETE k,c
        #     """, course_id=course_id, database="neo4j")
        #
        # df = pd.read_excel(file, header=None)
        #
        # # 创建或获取课程节点
        # course_exists = graph.execute_query("""
        ...
        nodes_data = [{"name": n[0], "level": n[1], "description": n[2], "pg_id": id_cache.get(n[0])} for n in nodes]
        belongs_data = [{"knowledge": r[0], "course": r[1]} for r in rel_belongs]
        sub_data = [{"child": r[0], "parent": r[1]} for r in rel_sub]
        precedes_data = [{"from": r[0], "to": r[1]} for r in rel_precedes]
        related_data = [{"from": r[0], "to": r[1]} for r in rel_related]
        ...
        def run_batch(tx):
            ...
            """, nodes=nodes_data, course_id=course_id)
    ...
    @staticmethod
    def add_knowledge_to_graph(name: str, course_id: int, description: str = None,
                               parent_id: int = None, driver=graph) -> KnowledgePoint:
        '''
        ...
        try:
            ...
            with driver.session(database="neo4j") as session:
                ...
                if parent:
                    ...
                    result = session.run("""
                        MATCH (p:Knowledge {name: $name}) RETURN p LIMIT 1
                    """, name=parent_name)
                    if result.single():
                        session.run("""
                        ...
                        """, parent_name=parent_name, child_name=name)
                    else:
                        print(f"[跳过图更新] 父节点 '{parent_name}' 不在图中，未建立父子关系")
                else:
                    # 创建 Course 节点
                    session.run("""
                        MERGE (c:Course {id: $course_id})
    ...
    def update_knowledge_point_node(kp_id: int, name: str, description: str, parent_id: int, course_id: int):
        # 删除原节点（按 id 匹配）
        graph.execute_query("""
                MATCH (k:Knowledge {name: $name}) DETACH DELETE k
            """, name=name)
    ...
    @staticmethod
    def delete_knowledge_point_node_from_graph(name: str):
